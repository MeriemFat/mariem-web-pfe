...
const PaymentFlouci = () => {
   ...
   const handleSumbitPayment = async (event) => {
      ...
      try {
         ...
         const payment = {
            userId: userId,
            buffetId: selectedBuffetId,
         };

         const response = await axios.post(`http://localhost:3000/payment/paymentFlouci/`, payment);
         ...
         window.location.href = `${paymentLink}?paymentId=${paymentId}`;
   ...
   return (
       <Fragment>
          <PageTitle activeMenu="Payment Flouci" motherMenu="Payment" />
